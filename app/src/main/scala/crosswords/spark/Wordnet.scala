...
/**
 * This class is a scala interface for WordNet corpus and methods like the words similarity.
 * It has been implemented by Sujit Pal. For details and explanations on how to install the libraries
 * see his blog at http://sujitpal.blogspot.ch/2014/04/nltk-like-wordnet-interface-in-scala.html
 * 
 */
class Wordnet(val wnConfig: File) {
  ...
  def synset(lemma: String, pos: POS, sid: Int): Option[Synset] = {
    val iword = dict.getIndexWord(pos, lemma)
    if (iword != null) Some(iword.getSense(sid))
    else None
  }

//  val synset = { (lemma: String, pos: POS, sid: Int) =>
//    val iword = dict.getIndexWord(pos, lemma)
//    if (iword != null) Some(iword.getSense(sid))
//    else None
//  }
