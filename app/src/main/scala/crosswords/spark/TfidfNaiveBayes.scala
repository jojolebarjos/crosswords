...
*/
  ...
  def extractFeatures(weightedData: RDD[(Seq[String], Seq[String], Float)], dict: RDD[(Long, String)]): RDD[LabeledPoint] = {
    ...
    // get the tfIdf of each character instead of frequency of each letter
    val toTfIdf = weightedData.map{ w => w._2.flatMap(d => d.toLowerCase.toList).map(x => x.toString) }
    //toTfIdf.saveAsTextFile("toTfIdf")
    val hashingTF = new HashingTF(26)
    val tf: RDD[Vector] = hashingTF.transform(toTfIdf)
    tf.cache()
    val idf = new IDF().fit(tf)
    val tfidf: RDD[Vector] = idf.transform(tf)

    // return the tfidfs directly as features, hoepfully it will be less biased
    weightedData.zip(tfidf).map { e =>
                                    // find the indices of the top tfidf words
                                    LabeledPoint(revDict(e._1._1(0)),e._2)
                                }
  }
