 ...
 * @author Utku Sirin
 */
object LetterFeaturesDecisionTree {
...
//           .set("spark.driver.maxResultSize","50g");
    ...
    val temp = weightedDataTemp.randomSplit(Array(0.70,0.30), seed = 11L);
    ...
    var auROC = 0.0
    val numIter = 1
    val numClasses = data.map(lp => lp.label).max + 1
    ...
    while(iter < numIter) {
        ...
        val model = DecisionTree.trainClassifier(training, numClasses.toInt, categoricalFeaturesInfo, impurity, maxDepth, maxBins)
